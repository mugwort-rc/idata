...
import csv
import idata.config
config = idata.config.load_yaml("config/e-stat.go.jp/kokusei/2015/lg/001.yml")
UNCIVILIZED_CP932 = "cp932"
source = config.load("data/e-stat.go.jp/kokusei/2015/lg/13/001_13.csv", encoding=UNCIVILIZED_CP932)
print(source.frame)
```

stdout:
```
地域コード 地域識別コード 境域年次(2015) 境域年次(2000)          name to21-01.0001  \
